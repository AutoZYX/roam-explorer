...
export default function KPIsContent({ showHeader = true }: { showHeader?: boolean }) {
  const { t } = useI18n();
        ...
        {KPIS.map((kpi) => (
          ...
          >
            <div className="flex items-start gap-3 mb-3">
              <span className="text-2xl font-bold text-[var(--accent)] w-8 shrink-0" style={{ fontFamily: "Playfair Display, serif" }}>
                {kpi.number}
              </span>
              <div className="flex-1">
                <h3 className="font-semibold text-[var(--text)]">{kpi.name}</h3>
